...
## DELETE Calculate P(ill) for entire trip
```{r calc_Pill_trip}
...
# MC draws: WQ conc, DW vol, Pillgiven inf
# Other point values: S, n_campingtrips, DW_campy, DW_giardia
...
calc_endpoints = function(S, n_campingtrips, DALY_weight, nMC,  # Point values
    ...
    # Wrangle data
    endpoints = data_frame(tripexposure, Pinf_trip, DALY_trip) %>%
        rename(
...
# Try the function
endpoints_campy = calc_endpoints(S = S, n_campingtrips = n_campingtrips, DALY_weight = DALY_campy, nMC = nMC,
...
```{r function to do one QMRA for lab vs field for one row of LRVs_forsummary sheet}

#### Do QMRA to compare lab vs field health risks 
# This function will be used in the next chunk in a for loop

# Inputs
#   nMC: the number of monte carlo draws to carry out
#   LRV_Ecoli_lab: point-value LRV for E. coli reductions in the lab (or manufacturer claim)
#   LRV_yeast_lab: point-value LRV for yeast reductions in the lab (or manufacturer claim)
#   LRV_Ecoli_field: point-value LRV for E. coli reductions in the field (obtained in 2021 field study)
#   LRV_yeast_field: point-value LRV for yeast reductions in the field (obtained in 2021 field study)
# Outputs
#   output: XXX To fill in

# Define values defined for our QMRA
S = 1
n_campingtrips = 1
DW_campy = 4.6/1000 # Units of DALY per illness case --> From WHO (2016), used in Bivins (2019)
DW_giardia = 1.7/1000   # Units of DALY per illness case --> From Health Canada (2012)


doQMRA_labvfield = function(nMC, LRV_manuf, LRV_field) {
