...
library(gghighlight)     # For graphing, to highlight one curve among the others
...
# For later: pivot longer for sensitivity analysis
# #     pivot_longer(
...
allQMRA_graph = allQMRA_long %>%
    pivot_wider(                    # Pivot wider so that manuf - field can be calculated
        ...
        values_from = endpoint_value
    ) %>%
    rename(                         # Rename columns to match our naming scheme
        endpoint_value_field = field,
        endpoint_value_manuf = manuf
    ) %>%
...
# So that if LRV_manuf > LRV_field, then the difference would be positive
...
### Graph setup 
# Establish colour palette, from Rcolourbrewer
brew_11 = c("#A50026",
            "#D73027",
            "#F46D43",
            "#FDAE61",
...
plot_subtractendp_exposure_g = ggplot(data = filter(allQMRA_graph, info_organism == "yeast"  & endpoint_name == "exposure"), 
                                    aes(endpoint_value_subtract, colour = info_treatmenttype, group = info_pairnum)) +
                            stat_ecdf(geom = "step", linewidth = 0.5, alpha = 0.5) +
                            facet_wrap( ~ info_treatmenttype, ncol = 1, 
                                    labeller = labeller(info_treatmenttype = treatment_type_labs)) +
